...
<body>
    <div class="container">
        ...
        <button class="btn btn-primary me-2" onclick="openMediaInsert()">新增圖片</button>
    </div>
        ...
        <div class="fixed-bottom bg-light p-3">
            <button id="confirmSelection" class="btn btn-primary">確認選擇</button>
        </div>
        <?php endif; ?>
    ...
    <script>
        function openMediaInsert() {
            window.open('mediaInsert.php?fromLibrary=true', 'MediaInsert', 'width=800,height=600');
        }

        <?php if ($selectMode): ?>
        document.getElementById('confirmSelection').addEventListener('click', function() {
            const selectedMedia = [];
            document.querySelectorAll('.media-checkbox:checked').forEach(function(checkbox) {
                selectedMedia.push({
                    id: checkbox.value,
            ...
            window.opener.postMessage({type: 'selectedMedia', media: selectedMedia}, '*');
            window.close();
        });
        <?php endif; ?>
    </script>
